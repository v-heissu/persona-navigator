...
class AutonomousNavigator:
    ...
    def start(self, url: str) -> Dict[str, Any]:
        ...
        """
        self.browser.start()
        screenshot, final_url = self.browser.navigate(url)
        ...
        result = self._get_persona_action(screenshot, page_type, final_url)
    ...
    def next_step(self) -> Dict[str, Any]:
        ...
        result = self._get_persona_action(screenshot, page_type, current_url)
        action = result.get("action", "DONE")
        target = result.get("target", "")

        # Esegui azione
        if action == "CLICK" and target:
            success, new_screenshot, new_url = self.browser.click_element(target)
            if success and self.state.should_visit(new_url):
                ...
                screenshot, current_url = self.browser.scroll_down()
            ...
            screenshot, current_url = self.browser.go_back()
            page_type = detect_page_type(screenshot, self.claude_client)
...
def execute_navigation_command(
    ...
    if action == "click":
        ...
        if selector:
            success, screenshot, new_url = browser.click_element(selector)
            if not success:
                # Prova con il testo del comando
                success, screenshot, new_url = browser.click_element(command)

    elif action == "goto":
        url = action_info.get("url", "")
        if url:
            screenshot, new_url = browser.navigate(url)

    elif action == "scroll_down":
        screenshot, new_url = browser.scroll_down()

    elif action == "scroll_up":
        screenshot, new_url = browser.scroll_up()

    elif action == "back":
        screenshot, new_url = browser.go_back()
